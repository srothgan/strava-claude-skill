...
- **Don't hardcode the unit system; confirm it per athlete.** Values are raw
  ...
  imperial account may surface miles/feet/degF -- verify rather than trust the label.
...
- **Elevation**: feet = metres x 3.281.
- **Speed (cycling display)**: km/h = m/s x 3.6; mph = m/s x 2.237.
...
- **Weight** is kg; convert to lb = kg x 2.205 if Imperial.
...
Use one of these templates unless the user asks for a different format. Keep the
same structure even when data is thin; missing data should be labeled, not hidden.
...
- **Distance / time / load:** `distance=<..>, moving_time=<..>, relative_effort=<..>`
- **Pace / power / HR:** `<pace or speed + HR/power deltas with context>`
- **Intensity profile:** `<laps or segments, zone interpretation>`
- **Limitations:** `<missing HR/power/temp, estimated values, small sample>`
- **Suggested next step:** `<one actionable question to test>`

### Weekly load / trend review

**Window:** `<4 weeks / 6 weeks / etc>`  
**Units:** `<metric/imperial + confirmed from profile>`  

- **Workload table:** include week-by-week `distance`, `moving_time`, `relative_effort`, `easy/hard split`
- **Trend summary:** `<up/down of distance and load>`
- **Risk / recovery flags:** `<one concise note>`
- **Recommendation:** `<2-3 practical adjustments>`
- **Confidence:** `<what supports this and what is uncertain>`
...
- **Current evidence:** `<best effort proxy from recent activities>`
- **Gap estimate:** `<high level range, not guaranteed outcome>`
- **Feasibility:** `<target timeline support with one sentence>`
- **What to improve next:** `<max 3 training priorities>`
- **What cannot be claimed:** `<hard limit of data>`
