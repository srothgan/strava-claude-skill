...
magnitudes. Units annotated below (metres, m/s, degC, etc.) describe the **common
...
- **Returns:** rich per-activity stats:
  ...
  - `laps` -- `[{ elapsed_time, moving_time (s), start_index, end_index,
    ...
    avg_grade (fraction; x100 for %) }]`. `start_index`/`end_index` map laps onto
...
- **Params:**
  ...
  - `streams` (array, required) -- any of: `time` (s from start), `location`
    ...
    `grade_smooth` (% grade), `moving` (bool), `temp` (degC)
